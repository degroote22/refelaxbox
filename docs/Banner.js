...
class ExampleBanner extends React.Component {
  ...
  render() {
    const { example, color, xray } = this.props;
    ...
    return (
      <Box style={sx.root}>
        <LiveProvider scope={scope} code={example} mountStylesheet={false}>
          <LiveError style={sx.error} />
          <Flex column style={sx.inner}>
            <Box auto style={sx.top}>
              <XRay color={textColor} backgroundColor={color} disabled={!xray}>
                <LivePreview style={sx.preview} />
              </XRay>
              <Btn
                style={sx.credits}
                href="http://jxnblk.com"
                children="Made by Jxnblk / Forked by degroote"
              />
            </Box>
